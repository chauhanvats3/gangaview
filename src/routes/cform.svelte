<script>
    ...
    const metadata = {
        url: "https://gangaview.com/cform",
        title: "CForm - Shri Ganga View Guest House",
        description: "Lets finalise your check in.",
        keywords:
    ...
    async function api_send_c_form() {
        try {
            ...
            mailProgress = "100";
            const mailURL = `/api/send-c-form-email?dataset=${JSON.stringify(
                $dataset
            )}`;
    ...
    function traverse(o, func, funcName) {
        for (let i in o) {
            ...
            if (o[i] !== null && typeof o[i] == "object") {
                ...
                } else traverse(o[i], func, funcName);
            }
        }
    }
...
<MailProgress
    bind:show={showMailProgress}
    bind:status={mailStatus}
    bind:progress={mailProgress}
    bind:mailError
    on:closeMe={() => {
...
<div class="cform">
    ...
    <div class="welcome" bind:this={welcome}>
        <p>Welcome to</p>
        <p
            class="brand gradient-text gradient-blue-pink noselect"
            on:click={() => {
                skipCheckCounter++;
            }}
        >
            Shri Ganga View Guest House
        </p>
        <p>Please Fill out the C-Form to Finalise your Check-In.</p>
    </div>

    <p class="caution">
        Avoid Using Device's Back Button Until Submit Is Successful.
    </p>

    <div class="entrySection" bind:this={entrySection}>
        {#if currentPage === "basic"}
            <Basic
                bind:data={$dataset.basic}
                bind:datavalid={$datavalid.basic}
            />
        {:else if currentPage === "passport"}
            <Passport
                bind:passport={$dataset.passport}
                bind:visa={$dataset.visa}
                bind:passdatavalid={$datavalid.passport}
    ...
    <Buttons
        on:buttonPressed={buttonPressed}
        {currentPageIndex}
        bind:showSubmit
    />
...
<style>
    ...
    :global(.invalid .form__field, .invalid .form__field:focus) {
